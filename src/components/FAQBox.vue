    ...
    <div v-for="(topic,index) in Topics" :key="index" class="Full w-full h-full flex flex-row justify-center content-center items-center p-3">
        ...
        <div v-else class=" flex flex-col justify-center content-center items-start w-full h-auto">
            <div  class="FullBox flex flex-col justify-center content-center items-center w-full h-full bg-orange-400  rounded ">
                ...
                <div class="dropDown flex flex-row justify-center content-center items-center h-1/6 w-full hover:bg-orange-300 active:bg-orange-500 duration-200" @click.prevent="() => toggleBox(index)">
                    <svg  class="rotate-180 m-5" width="23" height="13" viewBox="0 0 23 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M0.512163 1.18436C-0.138672 1.83521 -0.138672 2.89055 0.512163 3.54138L8.66599 11.687C9.96783 12.9877 12.0775 12.9872 13.3788 11.686L21.5294 3.53538C22.1803 2.88455 22.1803 1.82921 21.5294 1.17835C20.8785 0.527465 19.8233 0.527465 19.1724 1.17835L12.1963 8.15438C11.5455 8.80538 10.4902 8.80538 9.83933 8.15438L2.86916 1.18436C2.21833 0.533481 1.16316 0.533481 0.512163 1.18436Z" fill="#FAFAFA"/>
                    </svg>
                </div>
                <div class="Qlist flex flex-col justify-center items-center w-full bg-neutral-200">
                    <div v-for="(question, questionIndex) in topic.Questions" :key="question.ID" class="Questions  flex flex-row justify-between content-center items-center bg-neutral-200 w-full hover:bg-neutral-300 active:bg-neutral-100 duration-200 pt-3">
                        <div v-if="question.Opened === false" class="flex-1 flex justify-center" @click.prevent="() => toggleQuestion(index, questionIndex as number)">
                            <div class="flex-1 flex justify-center">
                                <h3 class="text-xl text-neutral-600 mb-3">{{ question.Question }}</h3>
                            </div>
                            <svg class="ml-auto mr-3 mt-3 mb-3" width="23" height="13" viewBox="0 0 23 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div v-else class="flex-1 flex flex-col justify-center content-center" @click.prevent="() => toggleQuestion(index, questionIndex as number)">
                            <div class="flex-1 flex justify-center content-center items-center">
                                <div  class="flex-1 flex justify-center">
                                    <h3 class="text-xl text-neutral-600 mb-3">{{ question.Question }}</h3>
                                </div>
                                <svg  class="ml-auto rotate-180 mr-3 mt-3 mb-3" width="23" height="13" viewBox="0 0 23 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path class="QdropDown fill-neutral-600" d="M0.512163 1.18436C-0.138672 1.83521 -0.138672 2.89055 0.512163 3.54138L8.66599 11.687C9.96783 12.9877 12.0775 12.9872 13.3788 11.686L21.5294 3.53538C22.1803 2.88455 22.1803 1.82921 21.5294 1.17835C20.8785 0.527465 19.8233 0.527465 19.1724 1.17835L12.1963 8.15438C11.5455 8.80538 10.4902 8.80538 9.83933 8.15438L2.86916 1.18436C2.21833 0.533481 1.16316 0.533481 0.512163 1.18436Z" fill="#FAFAFA"/>
                                </svg>
                            </div>
